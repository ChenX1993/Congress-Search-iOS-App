...
class FavoriteComController: UITableViewController, UISearchControllerDelegate, UISearchResultsUpdating, UISearchBarDelegate{
    var array = [JSON]()
    var array1 = [JSON]()
    var searchButton = UIBarButtonItem()
    var stopButton = UIBarButtonItem()
    let url = URL(string:"http://104.198.0.197:8080/committees?apikey=a32d292fdb6d4943a24e2b3d6ad53f9d&per_page=all")!
            ...
            case true:
                if let value = response.result.value {
                    ...
                    for j in json["results"].arrayValue {
                        if(databasecontroller.searchInCom(string: j["committee_id"].stringValue) > 0){
                            self.array.append(j)
                        }
                    }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        var viewcontroller = storyboard.instantiateViewController(withIdentifier: "FavoriteComDetail") as!ComJointDetailController
