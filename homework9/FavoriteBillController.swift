...
class FavoriteBillController: UITableViewController, UISearchControllerDelegate, UISearchResultsUpdating, UISearchBarDelegate {
    var legi = [JSON]()
    var legi1 = [JSON]()
    var array = [JSON]()
    var array1 = [JSON]()
    ...
    let url = URL(string:"http://104.198.0.197:8080/bills?history.active=true&last_version.urls.pdf__exists=true&apikey=a32d292fdb6d4943a24e2b3d6ad53f9d&per_page=50")!
     let url1 = URL(string:"http://104.198.0.197:8080/bills?history.active=false&last_version.urls.pdf__exists=true&apikey=a32d292fdb6d4943a24e2b3d6ad53f9d&per_page=50")!
            ...
            case true:
                if let value = response.result.value {
                    ...
                    for j in json["results"].arrayValue{
                        if(databasecontroller.searchInBill(string: j["bill_id"].stringValue) > 0){
                            self.array.append(j)
                        }
                    }
...
//            switch response.result.isSuccess {
//            case true:
//                if let value1 = response.result.value {
//                    let json1 = JSON(value1)
//                    for i in json1["results"].arrayValue{
//                        if(databasecontroller.searchInBill(string: i["bill_id"].stringValue) > 0){
//                            self.array.append(i)
//                        }
//                    }
//                    
...
//        }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        var viewcontroller = storyboard.instantiateViewController(withIdentifier: "favoriteBillDetail") as!BillDetailNewController
